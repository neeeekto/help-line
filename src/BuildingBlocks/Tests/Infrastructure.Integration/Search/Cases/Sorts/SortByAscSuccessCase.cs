...
{
    public class SortByAscSuccessCase : SortCaseBase
    {
        public override IEnumerable<TestModel> ExpectedResult => Data.OrderBy(x => x.Id).ToList();
        public override IEnumerable<Sort> Sorts => new[] {new Sort(true, nameof(TestModel.Id))};
    }
}
