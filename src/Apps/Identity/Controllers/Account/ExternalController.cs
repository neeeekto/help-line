...
using HelpLine.Modules.UserAccess.Application.Identity.Commands.AuthenticateByProvider;
...
namespace HelpLine.Apps.Identity.Controllers.Account
{
    ...
    public class ExternalController : Controller
    {
        ...
        public async Task<IActionResult> Callback()
        {
            ...
            if (user == null)
                throw new NotFoundException(providerUserId);
            ...
            await _events.RaiseAsync(new UserLoginSuccessEvent(provider, providerUserId, isuser.SubjectId,
                isuser.DisplayName,
        ...
        private async Task<(UserView user, string provider, string providerUserId, IEnumerable<Claim> claims)>
            FindUserFromExternalProvider(AuthenticateResult result)
        {
            ...
            var userId =
                await _userAccessModule.ExecuteCommandAsync(
                    new AuthenticateByProviderCommand(provider, providerUserId));
            // find external user
            var user = userId == null
                ? null
                : await _userAccessModule.ExecuteQueryAsync(new GetUserQuery((Guid) userId));
